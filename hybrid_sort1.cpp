 ...
 */
void merge_hybrid(std::vector<int>& nums, int left, int right, int mid){
 ...
 */
void insertion_sort_hybrid1(std::vector<int>& nums, int left, int right){
 ...
 */
void hybrid_sort_rec(std::vector<int>& nums, int left, int right,long h){
	...
	if(h > (right-left) + 1){
	  ...
	  hybrid_sort_rec(nums, left, mid,h); // sort first partition
	  hybrid_sort_rec(nums, mid+1, right,h); // sort second partition
	  merge_hybrid(nums, left, right, mid); // merge the two sorted partitions
	}
	else{
	  insertion_sort_hybrid1(nums, left, right);
 ...
 *Returns: void
 *
 *A function that calculates h = n^0.5 and 
 *calls recursive function hybrid_sort_rec
 *to sort using a merge sort, insertion sort hybrid
 *algorithm
 */
void hybrid_sort1(std::vector<int>& nums){
  
	unsigned long h = pow(nums.size(), 0.5);
	if(nums.size() > 1){
	
	  hybrid_sort_rec(nums,0,nums.size()-1,h);
	}
