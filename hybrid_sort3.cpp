 ...
 */
void merge_hybrid3(std::vector<int>& nums, int left, int right, int mid){
 ...
 */
void insertion_sort_hybrid3(std::vector<int>& nums, int left, int right){
 ...
 */
void hybrid_sort_rec3(std::vector<int>& nums, int left, int right,long h){
	...
	if(h > (right-left) + 1){
	  ...
	  hybrid_sort_rec3(nums, left, mid,h); // sort first partition
	  hybrid_sort_rec3(nums, mid+1, right,h); // sort second partition
	  merge_hybrid3(nums, left, right, mid); // merge the two sorted partitions
	}
	else{
	  insertion_sort_hybrid3(nums, left, right);
 ...
 *Returns: void
 *
 *A function that calculates h = n^0.1667 and 
 *calls recursive function hybrid_sort_rec3
 *to sort using a merge sort, insertion sort hybrid
 *algorithm
 */
void hybrid_sort3(std::vector<int>& nums){
  
	unsigned long h = pow(nums.size(), 0.1667);
	if(nums.size() > 1){
	
	  hybrid_sort_rec3(nums,0,nums.size()-1,h);
	}
